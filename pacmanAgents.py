...
class DQNAgent(Agent):
    ...
    def generate_report(self, filename="DQN_Pacman_Report.xlsx"):
        # Save rewards and loss
        df = pd.DataFrame({
            "Episode": list(range(1, len(self.reward_history)+1)),
            "Total Reward": self.reward_history,
            "Average Loss": [np.mean(self.loss_history)]*len(self.reward_history) if self.loss_history else [0]*len(self.reward_history)
        })
        ...
        plt.plot(df["Episode"], df["Total Reward"], marker='o', label="Total Reward")
        plt.title("DQN Pacman - Reward per Episode")
        plt.xlabel("Episode")
        plt.ylabel("Total Reward")
        plt.grid(True)
        plt.legend()
        plt.savefig("DQN_Pacman_Reward.png")
        plt.show()

        # Plot loss
        plt.figure(figsize=(10,5))
        plt.plot(df["Episode"], df["Average Loss"], marker='o', color='r', label="Average Loss")
        plt.title("DQN Pacman - Loss per Episode")
        plt.xlabel("Episode")
        plt.ylabel("Average Loss")
        plt.grid(True)
        plt.legend()
        plt.savefig("DQN_Pacman_Loss.png")
        plt.show()

        # Print summary
        print("\n===== Training Summary =====")
        print(f"Total Episodes: {len(self.reward_history)}")
        print(f"Average Reward: {np.mean(self.reward_history):.2f}")
        print(f"Max Reward: {np.max(self.reward_history):.2f}")
        print(f"Min Reward: {np.min(self.reward_history):.2f}")
        if self.loss_history:
            print(f"Average Loss: {np.mean(self.loss_history):.4f}")
